...
pub fn submit_app(
    ...
    let (status, submitted_by_key_id) = match &opt_key.0 {
        ...
        Some(key) => ("approved", Some(key.id.clone())),  // All submissions go live immediately
        None => ("approved", None),  // Anonymous submissions also go live immediately
    };
...
pub fn get_app(
    ...
    match result {
        Ok(app) => {
            // Record view for statistics (with optional key)
...
pub fn submit_review(
    ...
    // Upsert review (one review per key per app, or per anonymous user)
    let result = conn.execute(
...
pub fn create_key(
    ...
    // Admin keys can create admin keys, everyone else (including anonymous) can create regular keys
